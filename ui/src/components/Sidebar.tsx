...
const Sidebar: FC<SidebarProps> = ({ open, onClose }) => {
    ...
    return (
        <div>
            <div
                className={`bg-[var(--sketch-ink)]/20 fixed inset-0 z-[998] backdrop-blur-sm transition-opacity duration-300 ${
            ...
            >
                ...
                <div className="flex items-center justify-between border-b-2 border-dashed border-[var(--sketch-border)] p-4">
                    ...
                    <button onClick={onClose} className="sketch-btn p-1" aria-label="Close sidebar">
                        <XMarkIcon className="h-5 w-5" />
                    </button>
                </div>

                <nav className="p-4">
                    <p className="mb-3 text-xs font-semibold uppercase text-[var(--sketch-pencil)]">Quick Actions</p>
                    <ul className="space-y-2">
                        {navItems.map((item) => (
                            <li key={item.label}>
                                <a
                                    href={item.href}
                                    className="sketch-card hover:bg-[var(--sketch-highlight)]/30 flex items-center gap-3 p-3 transition-all hover:translate-x-1"
                                    onClick={onClose}
                                >
                                    <span className="text-2xl">{item.icon}</span>
                                    <div>
                                        <p className="font-semibold">{item.label}</p>
                                        <p className="text-xs text-[var(--sketch-pencil)]">{item.description}</p>
                                    </div>
                                </a>
                            </li>
                        ))}
                    ...
                    <ul className="space-y-1">
                        {extraItems.map((item) => (
                            <li key={item.label}>
                                <a
                                    href={item.href}
                                    className="hover:bg-[var(--sketch-highlight)]/20 flex items-center gap-3 rounded-md px-3 py-2 text-[var(--sketch-pencil)] transition-colors hover:text-[var(--sketch-ink)]"
                                    onClick={onClose}
                                >
                                    <span>{item.icon}</span>
